...
void ElegantOTAClass::begin(ELEGANTOTA_WEBSERVER *server, const char * username, const char * password) {
  ...
  #else
    _server->on("/ota/logo/light", HTTP_GET, [&](){
      _server->sendHeader("Cache-Control", "public, max-age=900");
      #if ELEGANTOTA_LOGO_GZIPPED == 1
        _server->sendHeader("Content-Encoding", "gzip");
      #endif
      _server->send_P(200, ELEGANTOTA_LIGHT_LOGO_MIME, (const char*)ELEGANTOTA_LIGHT_LOGO, sizeof(ELEGANTOTA_LIGHT_LOGO));
    });

    _server->on("/ota/logo/dark", HTTP_GET, [&](){
      _server->sendHeader("Cache-Control", "public, max-age=900");
      #if ELEGANTOTA_LOGO_GZIPPED == 1
        _server->sendHeader("Content-Encoding", "gzip");
      #endif
      _server->send_P(200, ELEGANTOTA_DARK_LOGO_MIME, (const char*)ELEGANTOTA_DARK_LOGO, sizeof(ELEGANTOTA_DARK_LOGO));
    });
  #endif
  ...
  #else
   _server->on("/ota/metadata", HTTP_GET, [&]() {
    // Create JSON object using printf-like formatting
    // the object should contain the following keys:
    // tt - title
    // hid - hardware ID
    ...
    _server->on("/ota/start", HTTP_GET, [&](AsyncWebServerRequest *request) {
      ...
      if (mode == OTA_MODE_FIRMWARE && !_fw_enabled) {
        ELEGANTOTA_DEBUG_MSG("OTA Mode: Firmware not enabled\n");
      ...
      if (mode == OTA_MODE_FILESYSTEM && !_fs_enabled) {
        ELEGANTOTA_DEBUG_MSG("OTA Mode: Filesystem not enabled\n");
    ...
    _server->on("/ota/start", HTTP_GET, [&]() {
      ...
      if (mode == OTA_MODE_FIRMWARE && !_fw_enabled) {
        ELEGANTOTA_DEBUG_MSG("OTA Mode: Firmware not enabled\n");
        return _server->send(400, "text/plain", "Firmware updates are disabled");
      }

      // Check filesystem mode
      if (mode == OTA_MODE_FILESYSTEM && !_fs_enabled) {
        ELEGANTOTA_DEBUG_MSG("OTA Mode: Filesystem not enabled\n");
        return _server->send(400, "text/plain", "Filesystem updates are disabled");
      }
...
void ElegantOTAClass::setTitle(const char * title){
...
void ElegantOTAClass::setFWVersion(const char * version){
...
void ElegantOTAClass::setFirmwareMode(bool enable){
  _fw_enabled = enable;
}

bool ElegantOTAClass::checkFirmwareMode(){
  return _fw_enabled;
}

void ElegantOTAClass::setFilesystemMode(bool enable){
  _fs_enabled = enable;
}

bool ElegantOTAClass::checkFilesystemMode(){
  return _fs_enabled;
}
